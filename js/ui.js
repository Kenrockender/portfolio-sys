...
function _updateThemePickerActive() {
  ...
  if (btn) {
    ...
    const icons = { dark:'🌙', light:'☀️', midnight:'🌊', forest:'🌿', dracula:'🧛', amoled:'⬛', solarized:'🌅', colorful:'🎨' };
    btn.textContent = icons[S.theme] || '🎨';
    btn.title = `Theme: ${cur?.label || S.theme}`;
  }
  // Update mobile button
  const mb = document.getElementById('mbtnTheme');
  if (mb) {
    const icons = { dark:'🌙 Dark', light:'☀️ Light', midnight:'🌊 Midnight', forest:'🌿 Forest', dracula:'🧛 Dracula', amoled:'⬛ AMOLED', solarized:'🌅 Solarized', colorful:'🎨 Colorful' };
    mb.textContent = icons[S.theme] || '🎨 Theme';
  }
}
...
function buildRow({ id, type, iconCls, icon, name, tagsHtml, sub, val, pct, barCol, liveStr, liveSt, metrics, yieldPct }) {
  ...
  return `<div class="asset-row" id="row-${id}" onclick="openAssetChart('${id}','${type}')" style="cursor:pointer">
    ...
    <div class="asset-actions" onclick="event.stopPropagation()">
      <button class="edit-btn" onclick="event.stopPropagation();openModal('${type}','${id}')" title="Edit">✎</button>
      <button class="del-btn" onclick="event.stopPropagation();del('${id}')" title="Del">✕</button>
    </div>
    <span class="asset-expand-chevron" style="opacity:.35">📈</span>
  </div>`;
}
...
export function renderCrypto(T) {
  ...
  if (assets.length === 0) {
    ...
    document.getElementById('cryptoRows').innerHTML = `<div class="no-results">${
      isSearch
        ? `🔍 Tidak ada kripto yang cocok dengan "<b>${S.searchQuery || S.filterPlatform}</b>"`
...
export function renderGold(T) {
  ...
  if (assets.length === 0) {
    ...
    document.getElementById('goldRows').innerHTML = `<div class="no-results">${
      isSearch ? '🔍 Tidak ada emas yang cocok' : '+ Tambah kepemilikan emas dengan tombol <b>Add</b>'
    }</div>`;
    return;
  }
...
export function renderStocks(T) {
  ...
  if (assets.length === 0) {
    ...
    document.getElementById('stocksRows').innerHTML = `<div class="no-results">${
      isSearch
        ? `🔍 Tidak ada saham yang cocok dengan "<b>${S.searchQuery || S.filterPlatform}</b>"`
  ...
  document.getElementById('stocksRows').innerHTML = Object.entries(groups).map(([ticker, entries]) => {
      ...
      return `<div class="stk-sub-row" id="subrow-${h.id}">
        ...
        <div class="stk-sub-right">
          ...
          <div class="asset-actions" style="opacity:1;position:relative;margin-left:4px;">
            <button class="edit-btn" onclick="event.stopPropagation();openModal('stocks','${h.id}')" title="Edit">✎</button>
            <button class="del-btn"  onclick="event.stopPropagation();del('${h.id}')" title="Del">✕</button>
          </div>
        </div>
      </div>`;
    }).join('') : '';
    ...
    return `<div class="stk-group" id="${groupId}">
      <div class="asset-row" onclick="${entries.length > 1 ? `toggleStkGroup('${groupId}')` : `openAssetChart('${entries[0].id}','stocks')`}" style="cursor:pointer">
        ...
        <div class="asset-actions" onclick="event.stopPropagation()">
          <button class="edit-btn" onclick="event.stopPropagation();openModal('stocks','${entries[0].id}')" title="Edit">✎</button>
          <button class="del-btn"  onclick="event.stopPropagation();del('${entries[0].id}')" title="Del">✕</button>
        </div>
        ${entries.length > 1
...
export function renderSavings(T) {
  ...
  if (assets.length === 0) {
    ...
    document.getElementById('savingsRows').innerHTML = `<div class="no-results">${
      isSearch ? '🔍 Tidak ada rekening yang cocok' : '+ Tambah rekening tabungan dengan tombol <b>Add</b>'
    }</div>`;
    return;
  }
...
export function renderAnalytics() {
  ...
  note.textContent = `📊 ${stats.dataPoints} snapshot historis`;
...
export function renderRebalance() {
  ...
  const classRows = classes.map(c => {
    ...
    const actionLabels = { ok: '✓ Seimbang', buy: '↑ Beli', sell: '↓ Jual' };
  ...
  const tickerRows = tickerList.map(a => {
    ...
    return `<div class="rebal-row">
      ...
      <div class="rebal-action ${actionCls[action]}">
        ${tgtPct > 0 && action !== 'ok' ? `<span class="rebal-badge-${action}">${diffIdr > 0 ? '↑ Beli' : '↓ Jual'}</span><span class="rebal-idr">${toDisp(Math.abs(diffIdr))}</span>` : action === 'ok' && tgtPct > 0 ? '<span class="rebal-badge-ok">✓</span>' : '–'}
      </div>
    </div>`;
  }).join('');
