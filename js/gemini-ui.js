...
window.sendGeminiMessage = async function () {
  ...
  try {
    ...
    document.getElementById(loadId)?.remove();
    appendGeminiMsg('model', `Error: ${e.message}. Cek API key dan koneksi internet.`);
  }

  btn.disabled    = false;
  btn.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" width="14" height="14"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg>';
};
...
window.triggerAutoAnalysis = async function () {
  ...
  try {
    ...
    document.getElementById(loadId)?.remove();
    appendGeminiMsg('model', `Analisis gagal: ${e.message}`);
  }
};
