...
function impSetStep(n){
  ...
  [1,2,3].forEach(i=>{
    ...
    if(i<n) el.classList.add('done'), document.getElementById('impStepNum'+i).textContent='✓';
      ...
      <div class="imp-source-row">
        ...
        <div class="form-group" style="margin:0">
          ...
          <div class="imp-tab-row" style="margin:0">
            <button class="imp-tab ${_impMethod==='text'?'active':''}" onclick="impSetMethod('text')">✏ ${impT('text','Text','Teks')}</button>
            <button class="imp-tab ${_impMethod==='pdf'?'active':''}"  onclick="impSetMethod('pdf')">📄 PDF</button>
            <button class="imp-tab ${_impMethod==='csv'?'active':''}"  onclick="impSetMethod('csv')">📊 CSV</button>
            <button class="imp-tab ${_impMethod==='ocr'?'active':''}"  onclick="impSetMethod('ocr')">🖼 ${impT('img','Image','Gambar')}</button>
          </div>
        </div>
      </div>
  ...
  area.innerHTML=`
    <div class="imp-template">
      <div class="imp-template-title">${impT('fmt','Recognized Format','Format yang Dikenali')}
        <button class="imp-copy-btn" onclick="impCopyTemplate()">📋 ${impT('copy','Copy example','Copy contoh')}</button>
      </div>
...
function impCopyTemplate(){
  ...
  if(t) navigator.clipboard.writeText(t.textContent).then(()=>{
    ...
    if(btn){ btn.textContent='✓ Copied!'; setTimeout(()=>btn.textContent='📋 Copy contoh',1500); }
  });
}
...
function applyAutoDetect(rawText) {
  ...
  if (fn) fn.textContent += ` — 🔍 Terdeteksi: ${IMPORT_RULES[detected]?.label || detected}`;
}
...
async function impHandlePdf(filesOrFile){
  ...
  if(fn) fn.textContent = files.length===1
    ? '📄 '+files[0].name
    : `📄 ${files.length} file (${totalPages} halaman)`;

  applyAutoDetect(_impRaw);
  fill.style.width='100%';
  txt.textContent=`✓ ${totalPages} halaman dari ${files.length} file diekstrak — ${_impRaw.length} karakter`;
...
async function impHandleOcr(filesOrFile){
  ...
  applyAutoDetect(_impRaw);
  fill.style.width='100%';
  txt.textContent=`✓ OCR selesai — ${_impRaw.length} karakter dari ${files.length} file`;
  setTimeout(()=>progress.style.display='none',2500);
}
...
async function impHandleCsv(filesOrFile) {
  ...
  for (let fi = 0; fi < files.length; fi++) {
    ...
    try {
      ...
      if (txt) txt.textContent = '⚠ Gagal: ' + file.name + ' — ' + e.message;
    }
  }
  ...
  if (txt) txt.textContent = `✓ ${_impRaw.length} karakter dari ${files.length} file CSV`;
