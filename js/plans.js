...
export const PRO_FEATURES = {
  ...
  // ai:         { label: 'AI Insights (Gemini)',   icon: _SVG.ai       }, // REMOVED
...
function _updatePlanUI() {
  ...
  if (badge) {
    if (_plan === 'pro') {
      ...
      badge.style.cssText = 'font-size:9px;padding:3px 8px;border-radius:5px;font-weight:800;letter-spacing:.1em;background:rgba(139,92,246,.2);color:#a78bfa;border:1px solid rgba(139,92,246,.4);display:inline-flex;align-items:center;cursor:default;'; // cursor:default = not clickable
      badge.onclick = null; // Remove click handler for Pro users
    } else {
  ...
  // AI button removed - no need to update
}
